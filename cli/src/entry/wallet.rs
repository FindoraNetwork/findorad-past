...
impl Wallets {
    ...
    pub fn read(&self, address: &str) -> Result<Wallet> {
        let addr = self
            ...
            .context("read detect_address failed")?;
        let result = self.wallets.iter().find(|w| w.address == addr);
        match result {
            Some(w) => Ok(w.clone()),
...
mod tests {
    ...
    fn test_entry_wallet() {
        ...
        let got = wallets
            .read("0x07fda471ddf402da8c3c4c9310efc8ddfcd517ea")
            .unwrap();
        ...
        let got = wallets
            .read("0x78e7b9d36ebe7a84898b9105d989d439d6b56adb")
            .unwrap();
